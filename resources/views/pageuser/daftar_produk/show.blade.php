    ...
    <div class="container py-5 my-5">
        <div class="row">
            <div class="col-md-6 mb-4">
                        ...
                        <div class="carousel-inner">
                            @foreach ($produk->gambar as $idx => $gbr)
                                <div class="carousel-item {{ $idx == 0 ? 'active' : '' }}">
                                    <img src="{{ asset($gbr) }}" class="d-block w-100 rounded"
                                        style="height: 600px; object-fit: cover;" alt="{{ $produk->nama_produk }}">
                                </div>
                            @endforeach
                        </div>
                ...
                @else
                    <img src="https://via.placeholder.com/600x800" class="img-fluid rounded border"
                        style="height: 600px; object-fit:cover; width:100%;" alt="">
                @endif
            </div>
            <div class="col-md-6">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('katalog.index') }}"
                ...
                <div class="p-2 d-flex flex-column justify-content-center">
                    ...
                    <h4 class="text-dark fw-bold mb-4" id="harga-display">
                        ...
                        @if ($produk->is_custom == 1 && $produk->harga_custom)
                            <br><small class="text-muted fs-6">Harga Custom: Rp
                                {{ number_format($produk->harga_custom, 0, ',', '.') }}</small>
                        @endif
                    </h4>
                    ...
                    <form action="{{ route('keranjang.add', $produk->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3" id="wrapper-varian">
                                ...
                                <div class="option-box-group" id="group-varian">
                                    @foreach ($produk->varian as $varian)
                                        <div class="option-box" data-group="varian" data-value="{{ $varian }}">
                                            {{ $varian }}</div>
                                    @endforeach
                                </div>
                                <input type="hidden" name="varian" id="input-varian" value="">
                                <div class="option-required-hint" id="hint-varian">⚠ Silakan pilih Warna terlebih dahulu
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="mb-3" id="wrapper-ukuran">
                                ...
                                <div class="option-box-group" id="group-ukuran">
                                    @foreach ($produk->ukuran as $ukuran)
                                        @php
                                            $stokUkuranIni = is_array($stokPerUkuran)
                                                ? $stokPerUkuran[$ukuran] ?? 0
                                                : 0;
                                        @endphp
                                        <div class="option-box {{ $stokUkuranIni == 0 ? 'out-of-stock' : '' }}"
                                            ...
                                            @if ($stokUkuranIni == 0)
                                                <small
                                                    style="font-size:0.65rem; display:block; margin-top:1px;">Habis</small>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                                <input type="hidden" name="ukuran" id="input-ukuran" value="">
                                <div class="option-required-hint" id="hint-ukuran">⚠ Silakan pilih ukuran terlebih dahulu
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($produk->is_custom == 1)
                            <div class="mb-3">
                                <label class="form-label fw-bold text-primary">Upload Desain Custom (Opsional / Wajib jika
                                    ingin custom)</label>
                                <input type="file" name="foto_custom" class="form-control rounded-0" accept="image/*">
                                <small class="text-muted"><i class="fa fa-info-circle"></i> Produk ini mendukung desain
                                    custom, silakan upload desain Anda.</small>
                            </div>
                        @endif
                        ...
                        <div class="row gx-2 mt-4">
                            <div class="col-sm-3 col-4 mb-2">
                                <input type="number" name="qty" id="input-qty"
                                    class="form-control form-control-lg rounded-0 text-center" value="1"
                                    min="1" max="{{ $hasUkuran ? 1 : $defaultStok }}">
                            </div>
                            <div class="col-sm-9 col-8 mb-2">
                                <button type="submit" id="btn-keranjang"
                                    class="btn btn-dark btn-lg rounded-0 w-100 fw-bold">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateStokUI(ukuran) {
                ...
                if (hargaDisplay) {
                    ...
                    const labelUkuran = (hargaKhusus && parseFloat(hargaKhusus) > 0) ?
                        ` <small class="text-danger fs-6 fw-normal">(harga ukuran ${ukuran})</small>` :
                        '';
                    hargaDisplay.innerHTML = `Rp ${formatted}${labelUkuran}`;
                }
            ...
            document.querySelectorAll('.option-box').forEach(function(box) {
                box.addEventListener('click', function() {
                    ...
                    document.querySelectorAll(`.option-box[data-group="${group}"]`).forEach(b => b
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    if (!valid) {
                        ...
                        if (firstHint) firstHint.scrollIntoView({
                            behavior: 'smooth',
                            block: 'center'
                        });
                    }
